...
export default function ChangeForm({projectId}){
  ...
  async function submit(e){e.preventDefault();if(busy)return;setBusy(true);const r=await fetch(`${API}/projects/${projectId}/changes`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({request})});const d=await r.json();if(d.id)location.href=`/projects/${projectId}/runs/${d.id}`;else alert(d.error||'Failed');setBusy(false)}
