...
async function startAction(run,action,input={}){
  const attempt=(await db.query('SELECT COALESCE(MAX(attempt),0)+1 attempt FROM run_actions WHERE run_id=$1 AND action=$2',[run.id,action])).rows[0].attempt;
  const row=(await db.query("INSERT INTO run_actions(run_id,action,attempt,status,source,input) VALUES($1,$2,$3,'running','manual',$4) RETURNING *",[run.id,action,attempt,input])).rows[0];
...
async function finishAction(actionRow,status,output={},error=null){
  return (await db.query('UPDATE run_actions SET status=$1,output=$2,error=$3,finished_at=NOW() WHERE id=$4 RETURNING *',[status,output,error,actionRow.id])).rows[0];
}
async function finishLatestRunningAction(runId,action,status,output={},error=null){
  const row=(await db.query("SELECT * FROM run_actions WHERE run_id=$1 AND action=$2 AND status='running' ORDER BY attempt DESC LIMIT 1",[runId,action])).rows[0];
  if(row) await finishAction(row,status,output,error);
}
async function writeFilesForRun(run,files=[],agent='coder',attempt=0,message){
  const dir=path.join(workspace,run.slug); await fs.mkdir(dir,{recursive:true});
  for(const f of files){const safe=path.normalize(f.path).replace(/^(\.\.(\/|\\|$))+/, ''); const full=path.join(dir,safe); if(!full.startsWith(dir)) throw new Error('Unsafe path'); await fs.mkdir(path.dirname(full),{recursive:true}); await fs.writeFile(full,f.content);}
  const git=simpleGit(dir); await git.add('.'); let sha=null;
  try{await git.commit(message||`agent: update run ${run.id}`); sha=(await git.revparse(['HEAD'])).trim();}catch{}
  if(sha && run.github_repo && process.env.GITHUB_TOKEN){const remote=`https://x-access-token:${process.env.GITHUB_TOKEN}@github.com/${run.github_repo}.git`; await git.push(remote,'HEAD:main',['--force-with-lease']).catch(()=>git.push(remote,'HEAD:main').catch(()=>{}));}
  await db.query('INSERT INTO code_versions(run_id,agent,attempt,files,commit_sha) VALUES($1,$2,$3,$4,$5)',[run.id,agent,attempt,files,sha]);
...
app.get('/runs/:id',async(req,res)=>{const run=(await db.query('SELECT r.*,p.name project_name,p.slug,p.github_repo,p.status project_status FROM runs r JOIN projects p ON p.id=r.project_id WHERE r.id=$1',[req.params.id])).rows[0]; if(!run)return res.sendStatus(404); const events=(await db.query('SELECT * FROM run_events WHERE run_id=$1 ORDER BY id',[run.id])).rows; const actions=(await db.query('SELECT * FROM run_actions WHERE run_id=$1 ORDER BY id',[run.id])).rows; res.json({...run,events,actions});});
app.post('/runs/:id/actions/:action/retry',async(req,res)=>{
  ...
  let run,actionRow;
  try{
    run=(await db.query('SELECT r.*,p.status project_status FROM runs r JOIN projects p ON p.id=r.project_id WHERE r.id=$1',[req.params.id])).rows[0];
    ...
    if(action==='planner') await db.query("UPDATE runs SET status='planning',current_stage='planner',plan=NULL WHERE id=$1",[run.id]);
    await event(run.id,run.project_id,'api','ACTION_RETRY_TRIGGERED',`${action} retry #${actionRow.attempt} sent to n8n`,{action,attempt:actionRow.attempt,webhook});
    ...
    if(run?.id){await db.query('UPDATE runs SET status=$1,current_stage=$2,finished_at=NOW() WHERE id=$3',['failed',action,run.id]).catch(()=>{});await event(run.id,run.project_id,'api','ACTION_RETRY_FAILED',`${action} retry could not be triggered`,{action,attempt:actionRow?.attempt,error:e.message}).catch(()=>{});}
...
app.post('/runs/:id/actions/:action/execute-retry',async(req,res)=>{
  ...
  let run,actionRow;
  try{
    run=(await db.query('SELECT r.*,p.slug,p.github_repo,p.status project_status FROM runs r JOIN projects p ON p.id=r.project_id WHERE r.id=$1',[req.params.id])).rows[0];
    if(!run)return res.sendStatus(404);
    if(run.project_status==='archived')return res.status(409).json({error:'Restore the project before retrying an action'});
    actionRow=(await db.query("SELECT * FROM run_actions WHERE run_id=$1 AND action=$2 AND attempt=$3 AND status='running'",[run.id,action,Number(req.body?.attempt)])).rows[0];
    if(!actionRow)return res.status(409).json({error:'Retry action was not registered or is no longer running'});

    let output={}; let nextStage=action; let nextStatus='running';
    if(action==='coder'){
      if(!run.plan) throw new Error('Coder requires an approved planner output');
      output=await postJson('http://agent-service:4100/coder',{request:run.request,plan:run.plan,humanFeedback:run.human_feedback});
      ...
      const dir=path.join(workspace,run.slug); await fs.access(path.join(dir,'package.json'));
      output=await postJson('http://runner:4200/execute',{projectSlug:run.slug,command:req.body?.command||'npm test'});
      await db.query('INSERT INTO execution_results(run_id,attempt,command,exit_code,stdout,stderr,duration_ms) VALUES($1,$2,$3,$4,$5,$6,$7)',[run.id,actionRow.attempt,req.body?.command||'npm test',output.exitCode,output.stdout||'',output.stderr||'',output.durationMs]);
      ...
      output=await postJson('http://agent-service:4100/reviewer',{request:run.request,plan:run.plan,execution:{exitCode:execution.exit_code,stdout:execution.stdout,stderr:execution.stderr,durationMs:execution.duration_ms},attempt:actionRow.attempt});
      ...
      output=await postJson('http://agent-service:4100/fixer',{request:run.request,plan:run.plan,execution:{exitCode:execution.exit_code,stdout:execution.stdout,stderr:execution.stderr},currentFiles:latestFiles,attempt:actionRow.attempt});
      ...
      const dir=path.join(workspace,run.slug); const git=simpleGit(dir); await git.add('.');
      try{await git.commit(`chore: manual git retry for run ${run.id}`);}catch{}
      const sha=(await git.revparse(['HEAD'])).trim();
      if(run.github_repo && process.env.GITHUB_TOKEN){const remote=`https://x-access-token:${process.env.GITHUB_TOKEN}@github.com/${run.github_repo}.git`; await git.push(remote,'HEAD:main',['--force-with-lease']).catch(()=>git.push(remote,'HEAD:main'));}
      output={commitSha:sha,pushed:Boolean(run.github_repo&&process.env.GITHUB_TOKEN)}; nextStage=run.current_stage==='git'?'report':run.current_stage;
      ...
      output={report,passed}; nextStage='report'; nextStatus=passed?'succeeded':'failed';
    ...
    await db.query("UPDATE runs SET status=$1,current_stage=$2,finished_at=CASE WHEN $1 IN ('succeeded','failed','cancelled') THEN NOW() ELSE NULL END WHERE id=$3",[nextStatus,nextStage,run.id]);
    await event(run.id,run.project_id,'api','ACTION_RETRY_COMPLETED',`${action} retry #${actionRow.attempt} completed`,{action,attempt:actionRow.attempt,status:actionStatus,nextStage});
    ...
    if(actionRow) await finishAction(actionRow,'failed',{},e.message).catch(()=>{});
    if(run?.id){await db.query('UPDATE runs SET status=$1,current_stage=$2,finished_at=NOW() WHERE id=$3',['failed',action,run.id]).catch(()=>{}); await event(run.id,run.project_id,'api','ACTION_RETRY_FAILED',`${action} retry failed`,{action,attempt:actionRow?.attempt,error:e.message}).catch(()=>{});}
    res.status(500).json({error:e.message,action,attempt:actionRow?.attempt});
  }
});
app.post('/runs/:id/plan',async(req,res)=>{const run=(await db.query('UPDATE runs SET plan=$1,status=$2,current_stage=$3,finished_at=NULL WHERE id=$4 RETURNING *',[req.body.plan,'awaiting_approval','human_approval',req.params.id])).rows[0]; await finishLatestRunningAction(run.id,'planner','succeeded',req.body.plan).catch(()=>{}); await event(run.id,run.project_id,'planner','PLAN_READY','Plan ready for manager approval',req.body.plan); res.json(run);});
...
app.post('/runs/:id/files',async(req,res)=>{try{const run=(await db.query('SELECT r.*,p.slug,p.github_repo FROM runs r JOIN projects p ON p.id=r.project_id WHERE r.id=$1',[req.params.id])).rows[0]; if(!run)return res.sendStatus(404); const result=await writeFilesForRun(run,req.body.files||[],req.body.agent||'coder',req.body.attempt||0,req.body.message); res.json(result);}catch(e){res.status(500).json({error:e.message})}});
